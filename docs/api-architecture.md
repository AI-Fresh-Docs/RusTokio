...
Если сценарий не обслуживает напрямую пользовательский интерфейс (frontend/admin UI), по умолчанию выбирается **REST**. GraphQL не используется как универсальный интеграционный слой для внешних и служебных клиентов.
