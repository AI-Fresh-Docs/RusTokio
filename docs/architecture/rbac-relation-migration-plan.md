...
- [x] **Фаза 2 — Единый Permission Resolver (завершено):**
  ...
  - Cache-aware relation resolve унифицирован в `rustok-rbac` через `resolve_permissions_with_cache` и контракт `PermissionCache`; `apps/server` использует `MokaPermissionCache` как инфраструктурный adapter, а логика cache hit/miss moved into module-level orchestration helper.
