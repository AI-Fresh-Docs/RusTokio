...
| `workflow` | admin | `in_progress` | `in_progress` | `crates/rustok-workflow/docs/implementation-plan.md` (slice: admin status/table/template presentation helpers вынесены в framework-agnostic core; Leptos layer потребляет view-models, GraphQL transport не изменялся) |
