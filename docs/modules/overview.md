...
Эти crate'ы не являются `RusToKModule`, но относятся к обязательной инфраструктуре платформы:
...
Итого обязательные **core-модули** платформы (`ModuleKind::Core`): `index`, `tenant`, `rbac`.
