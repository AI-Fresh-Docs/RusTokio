# Admin Migration Checklist (Legacy Admin → Leptos)

## Контекст и источники

Этот чеклист собран по двум источникам:

1. Матрица текущего статуса Admin в `docs/UI/developer-storefront-plan.md` (актуальные статусы функциональности для Phases 1–3).
2. Фактически реализованные маршруты/экраны Leptos-админки в `apps/admin/src`.

> В репозитории нет отдельного приложения legacy-админки в текущем дереве исходников; ниже "legacy" понимается зафиксированная ранее функциональность и пользовательские сценарии.
...
| Сценарий legacy-админки | Категория | Leptos статус | Примечание |
|---|---|---|---|
...
Legacy-админка может быть отключена только при выполнении всех условий:
