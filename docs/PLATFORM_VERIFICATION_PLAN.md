...
- [~] Нет бизнес-логики в resolvers — только вызов domain services
  - Основные mutation/resolver-ветки делегируют в сервисы; в `queries.rs` transport-shaping дальше вынесен в shared GraphQL types (`UserConnection::from_users`, `ActivityConnection::from_users`, `DashboardStats::from_metrics`). Но в `commerce/query.rs` и `queries.rs` всё ещё остаются прямые SeaORM-запросы и часть сборки response-моделей в resolver-слое
