...
- Starts the dedicated inventory write split with the native `inventory/variant/set-quantity` server-function endpoint, exposed through the inventory-owned API facade and backed by `InventoryService::set_inventory`.
...
- Writes targeted variant stock quantities through the dedicated native inventory facade; this write path has no GraphQL fallback and enforces tenant/permission checks server-side.
