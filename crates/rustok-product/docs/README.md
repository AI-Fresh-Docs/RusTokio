...
- module-owned `rustok-product/admin` и `rustok-product/storefront` теперь тоже
  ...
  framework-agnostic `admin/src/core.rs`, admin GraphQL операции проходят через
  module-owned facade `admin/src/transport.rs`, а Leptos render/effect adapter
  изолирован в `admin/src/ui/leptos.rs`;
